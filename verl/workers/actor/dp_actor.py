...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        # Allow trainer to override ppo_mini_batch_size per-call via meta_info
        # (e.g. on_policy_minibatch mode sets it to total_samples // world_size)
        ppo_mini_batch_size = data.meta_info.get(
        ...
        if "avg_response_tokens" in data.meta_info:
            ...
            self.config.global_batch_info["batch_num_tokens"] = (
                data.meta_info["avg_response_tokens"] * micro_bsz * dp_size
            )
            self.config.global_batch_info["dp_size"] = dp_size

        logger.warning("[update_policy] rank=%s, num_samples=%s, ppo_mini_batch_size=%s, "
                       "ppo_micro_batch_size_per_gpu=%s, ppo_max_token_len_per_gpu=%s",
                       torch.distributed.get_rank(), len(data), ppo_mini_batch_size,
                       self.config.ppo_micro_batch_size_per_gpu, self.config.ppo_max_token_len_per_gpu)
        ...
        if torch.distributed.get_rank() == 0:
            print(f"[update_policy] num_minibatches={len(mini_batches)}, total_batch_size={len(data)}")
        ...
        metrics = {
            "actor/pg_loss": 0.0,
            "actor/kl_loss": 0.0,
            "actor/num_minibatches": len(mini_batches),
            "actor/ppo_mini_batch_size": ppo_mini_batch_size,
            "actor/ppo_micro_batch_size_per_gpu": self.config.ppo_micro_batch_size_per_gpu,
            "actor/total_batch_size": len(data),
        }
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                if self.config.use_dynamic_bsz:
                    ...
                    ) # ppo_mini_batch_size should be adjusted to:
                    # number_of_prompts_to_cummulate * rollout_n // dp_size
                    # which is the effective number of responses we accumulate gradients for before stepping the optimizer
                ...
                for micro_idx, micro_batch in enumerate(micro_batches):
                    if torch.distributed.get_rank() == 0:
                        logger.info("[update_policy] minibatch %d/%d, microbatch %d/%d",
                                    batch_idx + 1, len(mini_batches), micro_idx + 1, len(micro_batches))
                    micro_batch = micro_batch.to(get_device_id())
                    micro_batch_metrics = {}
                    model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
        ...
        # # >>> DEBUG_POLICY_INTERMEDIATES BEGIN <<<
        # import pathlib, time as _time_mod
        ...
        #     "on_policy": on_policy,
        # }
        # _save_path = _debug_save_dir / f"update_policy_{_timestamp}_rank{_rank}.pt"
        # torch.save(_debug_payload, _save_path)
        # if _rank == 0:
        #     import dataclasses as _dc
        #     _cfg_path = _debug_save_dir / f"actor_config_{_timestamp}.pt"
        #     torch.save(_dc.asdict(self.config), _cfg_path)
        #     print(f"[DEBUG] Saved policy intermediates to {_save_path} "
        #           f"(shapes: old_log_prob={_debug_payload['old_log_prob'].shape}, "
        #           f"pg_loss={_debug_payload['pg_loss'].shape}), "
        #           f"actor config to {_cfg_path}")
        # # >>> DEBUG_POLICY_INTERMEDIATES END <<<
