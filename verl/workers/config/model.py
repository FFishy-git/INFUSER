...
from verl.utils.tokenizer import _try_resolve_hf_cache
...
class HFModelConfig(BaseConfig):
    ...
    def __post_init__(self):
        ...
        # Pre-resolve HF model IDs to local cache paths when available.
        # This avoids unnecessary HuggingFace API calls (e.g. model_info()
        # in transformers' _patch_mistral_regex) during tokenizer/config
        # loading, preventing 429 rate-limit errors when many processes
        # (e.g. vLLM replicas) initialize simultaneously.
        self.local_path = self._resolve_to_cache(self.local_path)

        # construct tokenizer
        if self.load_tokenizer:
            self.local_tokenizer_path = copy_to_local(self.tokenizer_path, use_shm=self.use_shm)
            self.local_tokenizer_path = self._resolve_to_cache(self.local_tokenizer_path)
        ...
        self.local_hf_config_path = self._resolve_to_cache(self.local_hf_config_path)
        ...
        # Post-resolve: after tokenizer/config downloads, paths that were
        # HF model IDs may now be resolvable to local cache directories.
        self.local_path = self._resolve_to_cache(self.local_path)
        if self.load_tokenizer:
            self.local_tokenizer_path = self._resolve_to_cache(self.local_tokenizer_path)
        self.local_hf_config_path = self._resolve_to_cache(self.local_hf_config_path)
    ...
    @staticmethod
    def _resolve_to_cache(path: str) -> str:
        """Resolve an HF model ID to its local cache path if cached."""
        if not path or os.path.isdir(path):
            return path
        resolved = _try_resolve_hf_cache(path)
        if resolved != path and os.path.isdir(resolved):
            return resolved
        return path
