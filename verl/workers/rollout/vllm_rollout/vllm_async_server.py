...
def _blackwell_compilation_kwargs(enforce_eager: bool) -> dict:
    """Return Blackwell-specific vLLM engine kwargs, else ``{}``.

    Mirrors ``verl_inf_evolve/sol_eval/vllm_runtime.py::_blackwell_cudagraph_kwargs``
    (duplicated here because verl is upstream of verl_inf_evolve and cannot
    import from it without a circular dependency).

    On SM>=10 (Blackwell: B100/B200/GB200/RTX 50xx/RTX Pro 6000) with
    ``enforce_eager=False``, force ``cudagraph_mode=PIECEWISE`` to dodge the
    vLLM 0.11.0 + FlashInfer FULL-decode CUDA graph capture assertion:

        File ".../vllm/v1/attention/backends/flashinfer.py", line 972, in forward
            assert decode_wrapper._sm_scale == self.scale

    Tracked upstream at https://github.com/vllm-project/vllm/issues/27057. The
    companion fix (``VLLM_USE_TRTLLM_ATTENTION=1``, needed so decode at
    inference time does not fall into the non-TRT-LLM path that asserts the
    same condition) lives in ``verl/trainer/constants_ppo.py`` so it is
    propagated to every Ray worker via ``runtime_env.env_vars``.

    Both workarounds can be removed once the ``LLM`` conda env upgrades to
    vLLM 0.11.1+. Note that upgrade hard-pins ``torch==2.9.0`` and
    ``flashinfer-python==0.5.2``; see ``docs/grace_sol_eval_setup.md`` for
    the env-rebuild plan.

    No-op on Hopper/Ampere/Ada (SM < 10) and when ``enforce_eager=True``.
    """
    if enforce_eager:
        return {}
    try:
    ...
    if major < 10:
        return {}
    logger.info(
        "Blackwell GPU detected (CC=%d.x); applying vLLM 0.11.0 cudagraph_mode="
        "PIECEWISE workaround. See _blackwell_compilation_kwargs docstring and "
        "https://github.com/vllm-project/vllm/issues/27057 for details.",
        major,
    )
...
class vLLMHttpServerBase:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        # Blackwell (SM>=10) + vLLM 0.11 cudagraph capture workaround. Merge
        # AFTER engine_kwargs so a user-supplied
        # ``config.engine_kwargs.vllm.compilation_config`` wins. No-op on
        # Hopper / Ampere / Ada or when enforce_eager=True.
        for _bk, _bv in _blackwell_compilation_kwargs(self.config.enforce_eager).items():
            args.setdefault(_bk, _bv)
...
class vLLMReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        for node_rank in range(nnodes):
            ...
            prefix = getattr(self, "server_name_prefix", "vllm_server")
            name = (
                f"{prefix}_{self.replica_rank}_{node_rank}"
                if not self.is_reward_model
                else f"{prefix}_reward_{self.replica_rank}_{node_rank}"
            )
