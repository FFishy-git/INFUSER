...
_WRAPPER_ACCEPTS_VLLM_CONFIG = "vllm_config" in inspect.signature(WorkerWrapperBase.__init__).parameters
...
class vLLMAsyncRollout(BaseRollout):
    ...
    def _init_zeromq(self) -> str:
        ...
        with FileLock(f"/tmp/verl_vllm_zmq_{getpass.getuser()}.lock"):
            ...
            if socket_type == "ipc":
                pid = os.getpid()
                # Use id(self) to disambiguate multiple rollout instances in the
                # same process (e.g., colocated generator + solver WorkerDict).
                address = f"ipc:///tmp/verl_vllm_zmq_{pid}_{id(self)}_{getpass.getuser()}.ipc"
            else:
    ...
    def _init_worker(self, all_kwargs: list[dict[str, Any]]):
        ...
        if _WRAPPER_ACCEPTS_VLLM_CONFIG:
    ...
    async def resume(self, tags: list[str]):
        ...
        if self.config.free_cache_engine:
            if self.inference_engine is not None:
                ...
                logger.warning("resume() called but inference_engine is None; skipping wake_up")
    ...
    async def release(self):
        ...
        if self.config.free_cache_engine:
            if self.inference_engine is not None:
                ...
                logger.warning("release() called but inference_engine is None; skipping sleep")
    ...
    async def update_weights(self, weights: Generator[tuple[str, torch.Tensor], None, None], **kwargs):
        ...
        if self.inference_engine is None:
            raise RuntimeError(
                "update_weights() called but inference_engine is None. "
                "This usually means the rollout was not shared between colocated roles "
                "(signature mismatch in _SHARED_VLLM_ROLLOUTS) and the vLLM server "
                "only initialized the other role's rollout. Check the 'Rollout signature' "
                "log messages to compare payloads between generator and solver roles."
            )
