...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        # 3. init trainer and rollout random states — derive from user
        # seed so reruns with the same ``rollout.seed`` are reproducible.
        self.torch_random_states = get_torch_device().get_rng_state()
        gen_dp_rank = rollout_device_mesh["dp"].get_local_rank()
        rollout_seed = int((self.config.rollout.get("seed", 42) if self.config.rollout else 42) or 42)
        get_torch_device().manual_seed(rollout_seed + gen_dp_rank)
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            total_global_tokens = data.meta_info["global_token_num"]
            if torch.is_tensor(total_global_tokens):
                total_global_tokens = int(total_global_tokens.sum().item())
            elif isinstance(total_global_tokens, np.ndarray):
                total_global_tokens = int(np.sum(total_global_tokens))
            elif isinstance(total_global_tokens, (list, tuple)):
                total_global_tokens = int(sum(total_global_tokens))
            else:
                total_global_tokens = int(total_global_tokens)
            logger.warning(
                "[ppo-worker-diag update_actor] role=%s, batch_size=%s, global_tokens=%s, "
                "ppo_mini_batch_size=%s, ppo_micro_batch_size_per_gpu=%s, update_policy_s=%.2f, "
                "mfu=%.4f, max_alloc_gb=%.2f, max_reserved_gb=%.2f, cpu_mem_gb=%.2f",
                self.role,
                len(data),
                total_global_tokens,
                data.meta_info.get("ppo_mini_batch_size", self.config.actor.ppo_mini_batch_size),
                self.config.actor.ppo_micro_batch_size_per_gpu,
                delta_time,
                metrics["perf/mfu/actor"],
                metrics["perf/max_memory_allocated_gb"],
                metrics["perf/max_memory_reserved_gb"],
                metrics["perf/cpu_memory_used_gb"],
            )
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        rank = dist.get_rank()
        log_with_rank(
            f"[actor-worker] save_checkpoint START (path={local_path}, step={global_step})",
        ...
        try:
            ...
            log_with_rank(
                f"[actor-worker] checkpoint_manager.save_checkpoint completed in {elapsed:.1f}s",
                rank=rank, logger=logger,
            )
        except Exception:
            elapsed = _time.time() - t0
            log_with_rank(
                f"[actor-worker] checkpoint_manager.save_checkpoint FAILED after {elapsed:.1f}s",
                rank=rank, logger=logger,
            )
            raise

        barrier_t0 = _time.time()
        dist.barrier()
        barrier_elapsed = _time.time() - barrier_t0
        if barrier_elapsed > 30.0:
            log_with_rank(
                f"[actor-worker] WARNING: post-save dist.barrier() took {barrier_elapsed:.1f}s "
                f"(possible rank stragglers)",
                rank=rank, logger=logger,
            )
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        logger.info(
            "[critic-worker rank=%d] save_checkpoint START (path=%s, step=%d)",
            rank, local_path, global_step,
        )
        ...
        try:
            ...
            elapsed = _time.time() - t0
            logger.info(
                "[critic-worker rank=%d] checkpoint_manager.save_checkpoint completed in %.1fs",
                rank, elapsed,
            )
        except Exception:
            elapsed = _time.time() - t0
            logger.exception(
                "[critic-worker rank=%d] checkpoint_manager.save_checkpoint FAILED after %.1fs",
                rank, elapsed,
            )
            raise
        ...
        if barrier_elapsed > 30.0:
            logger.warning(
                "[critic-worker rank=%d] post-save dist.barrier() took %.1fs (possible rank stragglers)",
                rank, barrier_elapsed,
            )
