...
def compute_grpo_outcome_advantage(
    ...
    Args:
        ...
        normalization_mode: `(Optional[str])`
            explicit normalization mode override. When set, takes precedence
            over ``norm_adv_by_std_in_grpo``. Supported values:
            - ``"group_std"``: standard GRPO (divide by per-group std)
            - ``"hybrid_std"``: divide by (per-group std + mean of all group stds)
            - ``"batch_std"``: subtract per-group mean, divide by std of the
              centered advantages across the whole batch
            - ``"none"``: subtract mean only (Dr.GRPO style)
    ...
    with torch.no_grad():
        ...
        if normalization_mode is None:
            eff_mode = "group_std" if norm_adv_by_std_in_grpo else "none"
        else:
            eff_mode = normalization_mode

        if eff_mode == "hybrid_std":
            # Mean of nonzero group stds — prevents extreme advantages when a
            # group has very low variance (v2_key_points #8).
            all_stds = torch.stack(list(id2std.values()))
            nonzero_stds = all_stds[all_stds > 0]
            avg_group_std = nonzero_stds.mean() if nonzero_stds.numel() > 0 else torch.tensor(0.0)
        elif eff_mode == "batch_std":
            # Std taken over the centered scores across the whole batch
            # (not per group). Matches inf_evolve/grad_utils/preprocessor.py
            # batch_std semantics.
            centered = torch.stack(
                [scores[i] - id2mean[index[i]] for i in range(bsz)]
            )
            batch_std = centered.std() if bsz > 1 else torch.tensor(0.0)

        for i in range(bsz):
            if eff_mode == "group_std":
                scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
            elif eff_mode == "hybrid_std":
                scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + avg_group_std + epsilon)
            elif eff_mode == "batch_std":
