...
### Integration Notes
- Used in `ray_trainer.py` / `self_evolution_trainer.py` via
  `compute_rollout_correction_and_add_to_batch()` (batch training pipeline).
- Used in `dp_actor.py` via `compute_rollout_corr_metrics_from_logprobs()`
  for per-micro-batch off-policy diagnostics during PPO training.
- All functions support batch inputs and valid token masking (via `response_mask`).

### Known Issue: Metric Key Collision
When both the trainer and actor compute off-policy metrics (KL, PPL, χ²), they produce
the same keys (e.g. `rollout_corr/chi2_seq`). After role-prefixing (e.g. `solver_`),
the actor-level values overwrite the trainer-level values during metric merging.
The actor values include training drift from optimizer steps, making chi2_seq
exponentially larger than the pre-training value. See `compute_offpolicy_metrics()`
...
def compute_offpolicy_metrics(
    ...
    .. warning:: **chi2_seq metric overwrite in multi-call pipelines**

        This function is called from two places with different ``old_log_prob`` inputs:

        1. **Trainer-level** (via ``compute_rollout_correction_and_add_to_batch``):
           Called once before PPO training starts. ``old_log_prob`` = recomputed log probs
           from the current checkpoint (no training drift). Measures the pure off-policy gap
           between the current checkpoint and the rollout policy.

        2. **Actor micro-batch loop** (via ``compute_rollout_corr_metrics_from_logprobs``
           in ``dp_actor.py``): Called per micro-batch during PPO training.
           ``old_log_prob`` = ``log_prob`` from the current forward pass, which changes
           after each ``optimizer.step()``. This conflates the off-policy gap with
           training drift accumulated across mini-batches.

        Both paths produce the same metric key (``rollout_corr/chi2_seq``). When logged
        with a role prefix (e.g. ``solver_``), the actor-level value **overwrites** the
        trainer-level value because the actor metrics are merged second.

        Because chi2_seq = E[exp(2·Σ_t log_ratio_t)] − 1 is **exponentially sensitive**
        to the variance of per-sequence log-ratio sums, even small policy drift from
        optimizer steps causes dramatic inflation. In practice this means the logged
        chi2_seq reflects *training-drift-inflated* divergence, not the pure off-policy
        gap. For example, with ~5800-token sequences, a 2× difference in ppo_kl
        (0.000135 vs 0.0000696) can produce a ~23× difference in chi2_seq
        (4030 vs 171).

    Args:
        old_log_prob: Log probabilities from training policy, shape (batch_size, seq_length).
            When called from the trainer, this is the recomputed checkpoint log probs.
            When called from the actor loop, this is the current forward-pass log probs
            (which drift as optimizer steps are applied across mini-batches).
...
def compute_rollout_corr_metrics_from_logprobs(
    ...
    This function is used in the actor micro-batch loop (``dp_actor.py``) to compute
    metrics using the CURRENT policy log probabilities versus rollout log probabilities,
    allowing tracking of the off-policy gap as training progresses.

    .. note:: **Metric key collision with trainer-level computation**

        The metrics returned here (e.g. ``rollout_corr/chi2_seq``) use the same keys
        as those computed by ``compute_rollout_correction_and_add_to_batch()`` at the
        trainer level. When both are logged with a role prefix (e.g. ``solver_``),
        the actor-level values **overwrite** the trainer-level values.

        Because ``log_prob`` here is the current forward-pass output (which drifts as
        ``optimizer.step()`` is applied between mini-batches), the chi2_seq metric
        reflects *training-drift-inflated* divergence rather than the pure off-policy
        gap. See the warning in ``compute_offpolicy_metrics()`` for details.

    Args:
        log_prob: Current policy log probabilities, shape (batch_size, seq_length).
            This drifts from ``old_log_probs`` as optimizer steps are applied across
            mini-batches within the PPO loop.
