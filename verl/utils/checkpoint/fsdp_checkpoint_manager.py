...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        def _ckpt_mem_diag(label: str) -> str:
            """Return a compact memory/disk diagnostic string."""
            parts = []
            try:
            ...
            try:
                ...
                parts.append(
                    f"sys_mem: used={sys_mem.used / 1024**3:.1f}G/{sys_mem.total / 1024**3:.1f}G ({sys_mem.percent}%)"
                )
                parts.append(f"disk_free={disk.free / 1024**3:.1f}G")
            except Exception:
                pass
            diag = ", ".join(parts)
            log_with_rank(f"[ckpt-worker {label}] {diag}", rank=self.rank, logger=logger)
            return diag
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_model:
                    _ckpt_mem_diag("pre-model-state-dict")
                    t0 = time.time()
                    model_state_dict = self.model.state_dict()
                    t1 = time.time()
                    log_with_rank(
                        f"[ckpt-worker] model.state_dict() took {t1 - t0:.1f}s",
                        rank=self.rank, logger=logger,
                    )
                    _ckpt_mem_diag("post-model-state-dict")
                    ...
                    log_with_rank(
                        ...
                        f"({fsize:.2f} GB, torch.save took {t1 - t0:.1f}s)",
                        rank=self.rank, logger=logger,
                    )
                    del model_state_dict
                    _ckpt_mem_diag("post-model-save-and-del")

                if self.should_save_optimizer:
                    _ckpt_mem_diag("pre-optim-state-dict")
                    t0 = time.time()
                    optimizer_state_dict = self.optimizer.state_dict()
                    t1 = time.time()
                    log_with_rank(
                        f"[ckpt-worker] optimizer.state_dict() took {t1 - t0:.1f}s",
                        rank=self.rank, logger=logger,
                    )
                    _ckpt_mem_diag("post-optim-state-dict")

                    t0 = time.time()
                    torch.save(optimizer_state_dict, optim_path)
                    t1 = time.time()
                    fsize = os.path.getsize(optim_path) / 1024**3
                    log_with_rank(
                    ...
                    _ckpt_mem_diag("post-optim-save-and-del")
        ...
        total_elapsed = time.time() - save_t0
        _ckpt_mem_diag("post-all-saves")
        log_with_rank(
            f"[ckpt-worker] all state saves completed in {total_elapsed:.1f}s",
            rank=self.rank, logger=logger,
        )
        ...
        if self.should_save_hf_model:
            # Free lingering CPU memory from the FSDP shard saves above
            # before the full state-dict allgather allocates a large
            # contiguous buffer on rank 0.  This is especially important
            # in colocated setups where generator + solver share workers
            # and a previous save's state_dict may not yet be reclaimed.
            import gc as _gc
