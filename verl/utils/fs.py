...
def _try_resolve_hf_cache(name_or_path: str) -> str:
    """Resolve a HuggingFace model ID to its local cache snapshot if available.

    Returns the local snapshot path when the HF hub cache contains the model,
    otherwise returns *name_or_path* unchanged.
    """
    if os.path.isdir(name_or_path):
        return name_or_path
    hf_home = os.environ.get("HF_HOME", os.path.expanduser("~/.cache/huggingface"))
    snapshots_dir = os.path.join(hf_home, "hub", f"models--{name_or_path.replace('/', '--')}", "snapshots")
...
def copy_to_local(
    ...
    # For HF model IDs that copy_local_path_from_hdfs returned unchanged,
    # resolve to the local HF hub cache when available.  This avoids HF API
    # calls during model/tokenizer loading and prevents rate-limit-induced
    # inconsistencies across Ray workers.
    if local_path == src and not os.path.isdir(local_path) and not is_non_local(src):
        resolved = _try_resolve_hf_cache(src)
        if resolved != src:
            local_path = resolved
